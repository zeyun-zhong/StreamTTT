#!/bin/bash
# StreamingBench RTVU subset (paper Table 1).
# Usage: DATA_DIR=... CKPT=... bash scripts/eval/streamingbench.sh
set -euo pipefail
...
torchrun --standalone --nproc_per_node="${NPROC}" -m evaluation.streamingbench.distributed_evaluate_streamingbench \
  --model_type streamttt \
  --model_name_or_path "${CKPT}" \
  --benchmark_path "${DATA_DIR}/StreamingBench" \
